...
@section('content')
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h3 style="color: #4b5563; font-weight: 600;">Editar Categoría: {{ $category->name }}</h3>
    ...
    <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); max-width: 600px;">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" style="background: transparent; padding: 0; box-shadow: none;">
            @csrf
            @method('PUT')
            ...
            <textarea name="description" id="description" rows="4" 
                      style="background: #f9fafb; color: #1f2937; border: 1px solid #d1d5db; width: 100%;">{{ old('description', $category->description) }}</textarea>

            <button type="submit" style="background-color: #38bdf8; color: white; padding: 12px 20px; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; width: 100%; margin-top: 20px;">Actualizar Categoría</button>
        </form>
    </div>
